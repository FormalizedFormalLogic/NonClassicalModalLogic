...
lemma curry (h : ⊢ᴴ[CK;𝔸] A ⋏ B 🡒 C) : ⊢ᴴ[CK;𝔸] A 🡒 B 🡒 C :=
  imp_trans andIntro (imp_comp_left h)

lemma uncurry (h : ⊢ᴴ[CK;𝔸] A 🡒 B 🡒 C) : ⊢ᴴ[CK;𝔸] A ⋏ B 🡒 C :=
  mdp_ctx (imp_trans andElim₁ h) andElim₂

lemma and_mono_left (h : ⊢ᴴ[CK;𝔸] A 🡒 B) : ⊢ᴴ[CK;𝔸] A ⋏ C 🡒 B ⋏ C :=
...
lemma fconj_insert_mono (h : ⊢ᴴ[CK;𝔸] B 🡒 A) : ⊢ᴴ[CK;𝔸] ⋀(insert B Γ) 🡒 ⋀(insert A Γ) :=
...
lemma fconj_union : ⊢ᴴ[CK;𝔸] ⋀(Γ₁ ∪ Γ₂) 🡒 ⋀Γ₁ ⋏ ⋀Γ₂ :=
