...
lemma counterModel_not_bBox : ¬counterModel.BBox := by
  rintro ⟨h⟩;
...
theorem exists_symmetricMRel_not_bBox :
  ∃ (κ : Type) (F : Frame κ), F.SymmetricMRel ∧ ¬F.BBox :=
  ⟨Fin 3, BBox.counterModel.toFrame, inferInstance, BBox.counterModel_not_bBox⟩
